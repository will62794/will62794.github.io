...
Google's [Percolator system](https://www.usenix.org/legacy/event/osdi10/tech/full_papers/Peng.pdf) implements a standard lock-based implementation of SI, which adds *lock* and *write* columns, where the *write* column maintains the commit timestamp. It runs a 2PC algorithm that first writes the data and acquires the corresponding locks.

A basic lock-free implementation of snapshot isolation can be done using a centralized *status oracle*, that is responsible for receiving commit requests from transactions and checking for conflicts.
...
The paper next examines the question of what role write-write conflict play in snapshot isolation and serializability. The most standard example of non-serializable snapshot isolation histories are those of *write skew*, where transactions don't write to conflicting keys, but both read from a set of keys that the other writes to.

But aborting transactions on write-write conflict is also overly restrictive in some ways i..e they will abort transactions in some cases even if an anomly would not manifest. They consider a modified variant of *lost update* anomaly, like
...
Overall, this paper provides a nice perspective on snapshot isolation in general, and re-considering its underlying assumptions. One takeaway for me is a reinforcement of the somewhat arbitrary delineations between isolation levels. For example, I think that snapshot isolation is in one way intuitive i.e. every transaction reads from a consistent snapshot, but the whole concept of write-write conflicts actually seem a bit arbitrary. This paper kind of sheds light on that by showing that, in some sense, read-write conflicts are actually the more "natural" type of thing you would want, at least in the sense that they give you a more fundamental guarantee i.e. serializability. I'm not sure that the anomalies allowed under SI (i.e. write skew) are really intuitive in any fundamental way.
