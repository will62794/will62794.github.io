---
layout: post
title:  "Transformers not Transactions"
...
For transactions oeprating at isolation levels that read from a consistent database snapshot, though, I would argue that transactions really don't need to be modeled as linear sequences of program steps, and can alternately be viewed as *state transformers*. That is, we can compact any transaction into the following object, illustrated by example, for a database with keys $$K=\{x,y,z\}$$:
...
Notably, a lot of traditional transaction models don't make this operational view explicit in their models. For example, consider the way that papers treat the *lost update* anomaly. In Cerone 2015 paper, their model represents transactions as sequences of read/write operations over a set of keys, e.g.
...
Going further, this transfomer model also gives us a nice way to see that *write skew*, which is another anomaly that is allowed under full snapshot isolation, is just a generalization of lost update and is clear to see this in the transformer model. Essentially, write skew exists because two transactions don't write to overlapping key sets, but they both update keys in a way that breaks a semantic constraint. Again, the transformer model representation for the cannonical write skew anomaly looks like:
