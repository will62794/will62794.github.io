...
where $$\overline{P}$$ is the smallest safety property that contains $$P$$, and $$L=\neg(\overline{P}-P)$$.The topological characterization of safety and liveness properties is elegant, but I don't think it's the clearest way to understand the topic for someone without a strong prior intuition in topology. We can prove the theorem without resorting to the topological characterization, though.
...
Here is one other, possibly more intuitive way to think about the safety-liveness decomposition at a high level. To construct an arbitrary property $$P$$, we start by plucking out all the "safe" prefixes, which gives us our safety property. This leaves us with some behaviors that are "safe" but may still violate our property. This is the set $$\overline{P}-P$$ from above. We need a liveness property $$L$$ to fix this. Note that, by definition, any liveness property must include all finite prefixes, whether they are safe or not, so this is not a configurable parameter of the liveness property we choose. The simplest liveness property we could choose is the one that includes all behaviors, but this will obviously not work since it will include the behaviors in $$\overline{P}-P$$. Ideally, we want to remove from $$L$$ exactly the behaviors in $$\overline{P}-P$$. After removing them, though, will we still have a liveness property? As we noted, all finite prefixes must be contained in a liveness property, so we can remove individual behaviors but we cannot remove so many behaviors such that a particular finite prefix is no longer represented in the liveness property. Remember that the liveness definition only specifies that every partial execution has *some* extension in $$L$$. It doesn't care how many there are. As long as there is at least one extension of every finite prefix contained in the liveness property, we're ok. So, the question, then, is if we removed all the behaviors in $$\overline{P}-P$$ from $$L$$, would we still have a liveness property? Well, the prefixes contained in $$\overline{P}-P$$ are all safe prefixes, by definition. So, we know that all of these prefixes must also appear in $$P$$, since $$P$$ will contain all safe prefixes. So, removing all the behaviors from $$\overline{P}-P$$ from our liveness property doesn't change the set of prefixes contained in $$L$$, so it will still be a liveness property, and we know that is intersection with our safety property gives us exactly $$P$$.
