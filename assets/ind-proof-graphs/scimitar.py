...
#
    ...
    def has_path(self, graph, start, end, visited=None):
        if visited is None:
            visited = set()
        if start == end:
            return True
        ...
        for edge in self.proof_graph["proof_graph"]["edges"]:
            if edge[0] == start and edge[1] not in visited:
                if self.has_path(graph, edge[1], end, visited):
                    return True
        return False
    ...
    def render_proof_graph(self, save_tex=False, include_seed=True):
            ...
            for n in e:
                ...
                if n in self.proof_graph["nodes"]:
                    ...
                    if "is_lemma" in node:
                        ...
                        label = "< " + "<FONT POINT-SIZE='28'>" + name_parts[0]  + "</FONT>"
                        depth_str = ""
                ...
                dot.node(n, fillcolor=fillcolor, style=style, color=color, label=label, shape=shape, fontsize=fontsize, penwidth=penwidth)
        ...
        if save_tex:
            ...
            texcode = dot2tex.dot2tex(dot.source, 
                                      debug=False, 
                                      output="dot2tex.log", 
                                      format='tikz', figpreamble=figpreamble, 
                                      autosize=True, crop=False, 
                                      figonly=True, texmode="math",
                                      graphstyle="scale=1.2")
    ...
    def run(self):
        ...
        total_dur_precheck = self.total_duration_secs

        # If we have found what seems to be a valid proof graph, re-check it just to be sure.
        if self.proof_tree_mode and len(self.proof_graph_failed_lemma_nodes()) == 0 and not self.recheck_proof_graph and not self.persistent_proof_tree_mode:
        ...
        logging.info(f"Total duration pre proof graph check: {total_dur_precheck:.2f} secs.")
