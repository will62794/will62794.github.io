...
This lookup table gave me the ability to efficiently find the seed for any given 31-bit sequence prefix. To find the reseed positions in $$GambleSeq$$, I scanned through $$GambleSeq$$ looking at subsequences of length 31 and looking up their corresponding seed in the table. I would then check to see if the seed produced the correct sequence beyond the 31 element subsequence. If it didn't, I assumed it was a false positive i.e. it wasn't actually a reseed point. Unfortunately, this approach didn't turn up the results I was hoping for. I wasn't able to find points in the sequence where a seeds produced a subsequence that extended beyond 31 elements.
