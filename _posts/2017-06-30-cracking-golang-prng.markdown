...
The structure of the app was very simple. There was a single server that handled incoming requests from users to transfer money between each other, or to execute new gambles. There was a backing  database that stored user information and logged operations. Every gamble that occurred, and its outcome, would be saved, along with a timestamp of when that gamble occurred. Users could interact with the app via a Slack bot or a web interface. By scraping the public API, it was possible to obtain a full, ordered list of all gambles that ever occurred. The application server used a single, global random number generator to simulate the virtual coin tosses, and so my initial hypothesis was very simple: every gamble corresponds to one generation of the app's internal random number generator. This formed the basis of my initial approach: given a sequence of values generated by the internal random number generator, figure out a way to predict the next value that would be generated. This would allow me to predict the outcome of any gamble with perfect accuracy. In the rest of this post, I will refer to the sequence of gambles as a binary sequence, since each gamble, and corresponding random number generator output, is either heads or tails i.e. 0 or 1.
...
<small>*Mongobucks Slack bot interface*</small>
...
With the lookup table built, I could efficiently find the seed for any given 31-bit sequence prefix. I needed to find the reseed positions in $$GambleSeq$$, so I scanned through looking for subsequences of length 31 that mapped to valid seeds. If a subsequence mapped to a valid seed, then I would check to see if the seed produced the correct sequence beyond the 31 element subsequence. If it didn't, I assumed it was a false positive. That is, that wasn't actually a reseed point. Unfortunately, this approach didn't turn up the results I was hoping for. I wasn't able to find points in the sequence where a seeds produced a subsequence that extended beyond 31 elements. 
