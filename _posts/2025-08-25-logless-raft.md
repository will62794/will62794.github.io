...
When a new leader gets elected, a "fork" may be created in this tree, if the new leader did not contain all previously created (but uncommitted) log entries. For example, this first occurs in State 8, when node `n2` becomes leader but without the log entry `(4,1)` created by `n1`. Similarly, another fork is created when a branch via `n3` is created in State 14. 
